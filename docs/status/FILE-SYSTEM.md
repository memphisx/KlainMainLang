...
| Async variants (`fs.readFile`, callback/Promise-based) | ❌ | | • Everything here is synchronous and blocking by design; the event loop + streams now exist ([TDD-00097](../tdd/TDD-00097.md)), so non-blocking variants (and `fs.createReadStream`/`createWriteStream`) are implementable when prioritized |
