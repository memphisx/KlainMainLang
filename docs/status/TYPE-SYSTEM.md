...
**Caveats**: Union types beyond `T | null` aren't supported (the parser discards every union member except the first) — see [ADR-00008](../adr/ADR-00008.md). `any`/`unknown` are Staged V1: declare/assign/reassign/print/`typeof`/`===` work, but arithmetic and use as a function param/return/array/object-field type are a clean compile error (except a `@erased` generic function's own bare-`T` positions — see below). User-defined generics (`function identity<T>`, `interface Box<T>`, `class Box<T>`) are limited to a single, unconstrained type parameter with no explicit call-site type arguments (`identity<number>(5)`) or constraints, regardless of whether a declaration uses default monomorphization (functions need an inferable `T`/`T[]`-typed parameter to infer from; classes need an explicit `new Box<T>(...)` type argument) or the opt-in `@erased` type-erasure escape hatch, which is narrower still — functions only, and only a bare `T` parameter/return position, not `T[]`. See [TDD-00010](../tdd/TDD-00010.md); multiple type parameters (`<K, V>`) are scoped as a follow-on, not started — see [TDD-00037](../tdd/TDD-00037.md).
