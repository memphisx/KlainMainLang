...
**Coverage**: 3/4 (~75%) · **Strict Coverage**: 1/4 (~25%).
...
| `queueMicrotask(fn)` | ❌ | | • Microtask queue (also a JS language global)<br>• A real microtask queue (distinct from the timer queue) is a bigger, separate design than `setImmediate` was — see [TDD-00002](../tdd/TDD-00002.md) for the timer design |
