...
**Strict Coverage note**: the "Strict" column below is each linked page's own overall Strict Coverage stat (a row counts only with zero known caveats/bugs, repro-verified — see the 2026-08-11 audit, [ADR-00166](../adr/ADR-00166.md)), not a per-category split — several categories below share one page (e.g. every `LANGUAGE-CONSTRUCTS.md` row shows that page's single 34/63 figure), so treat it as a page-level signal, not an exact per-row count.

| Category | Coverage | Strict | Page | Caveats |
|---|---|---|---|---|
| Control flow statements | 11/11, 100% | 34/63, ~54% (page) | [LANGUAGE-CONSTRUCTS.md](LANGUAGE-CONSTRUCTS.md) | A `for` loop's update clause supports comma-separated expressions (`i++, j--`) — see [ADR-00156](../adr/ADR-00156.md) — but a non-declaration init clause (`for (i = 0, j = 10; ...)`) is still a clean rejection; no ASI restriction on a line break before a postfix `++`/`--`. `try`/`catch`/`finally`: `finally` does not run if `try`/`catch` itself returns — see [ADR-00166](../adr/ADR-00166.md) |
| Operators | 42/42, 100% | 34/63, ~54% (page) | [LANGUAGE-CONSTRUCTS.md](LANGUAGE-CONSTRUCTS.md) | `**` doesn't exist at the lexer level; `&&`/`\|\|` don't actually short-circuit (both operands always evaluate); `NaN`'s `!=`/`!==` comparisons are wrong — all found by the 2026-08-11 audit, see [ADR-00166](../adr/ADR-00166.md) |
| Variable declarations | 4/4, 100% | 34/63, ~54% (page) | [LANGUAGE-CONSTRUCTS.md](LANGUAGE-CONSTRUCTS.md) | Multi-declarator `let`/`const`/`var` (`let i = 0, j = 10;`) is now done — see [ADR-00156](../adr/ADR-00156.md) — but `const` doesn't reject a missing initializer, and `arguments`/`eval` aren't reserved as binding names |
| Functions & closures | 11/11, 100% | 34/63, ~54% (page) | [LANGUAGE-CONSTRUCTS.md](LANGUAGE-CONSTRUCTS.md) | Nested function declarations are V1-scoped (no closure capture of the enclosing body's locals, one block deeper than the enclosing body is unsupported) — see [TDD-00057](../tdd/TDD-00057.md); tagged template literals have no `.raw` property on the `strings` array (real `String.raw` stays separately out of scope, see [ADR-00028](../adr/ADR-00028.md)) — see [TDD-00059](../tdd/TDD-00059.md)/[ADR-00152](../adr/ADR-00152.md). A top-level function can't reference a sibling top-level `let`/`var`/`const` at all; a default parameter can't reference an earlier parameter; a rest-only arrow parameter list fails to parse — all found by the 2026-08-11 audit, see [ADR-00166](../adr/ADR-00166.md) |
| Type primitives | 9/14, ~64% | 13/20, 65% (page) | [TYPE-SYSTEM.md](TYPE-SYSTEM.md) | No `bigint` |
| Async / Promise | 4/9, ~44% | 34/63, ~54% (page) | [LANGUAGE-CONSTRUCTS.md](LANGUAGE-CONSTRUCTS.md) | Only `await fetch(...)` is genuinely non-blocking; every other `Promise<T>` is a resolved-slot read |
...
| Object & collections | 23/26, ~88% | 12/26, ~46% | [OBJECT-COLLECTIONS.md](OBJECT-COLLECTIONS.md) | No `WeakMap`/`WeakSet`/`WeakRef`, `Object.create`/`.fromEntries`; `new Set(iterable)` now accepts an initial-elements array (see [ADR-00159](../adr/ADR-00159.md)), but `new Map(entries)` still doesn't — each entry would need a real `[K, V]` tuple type, which this compiler doesn't have. `Map<string,number>.get()` on a missing key silently returns `0`, not `undefined` — found by the 2026-08-11 audit |
| JSON | 9/11, ~82% | 7/9, ~78% | [JSON.md](JSON.md) | No nested-object `JSON.parse`; array-typed interface fields fail to compile instead of a clean rejection (known limitation). `JSON.parse` into a `float64`/`float32`-typed variable fails to compile — found by the 2026-08-11 audit |
...
| Function expressions (`var f = function() {...}`, IIFEs `(function(){...})()`) | Only function *declarations* (`function f() {}`) and arrow functions are expressions today — `function` as an expression (assigned to a variable, passed inline, or immediately invoked) is a clean parse-time rejection, not a silent misparse. By far the single largest blocker found running the [Test262 conformance harness](../testing/CONFORMANCE-RESULTS.md) unfiltered against this compiler ([TDD-00008](../tdd/TDD-00008.md) Design V2, [ADR-00152](../adr/ADR-00152.md)): ~13,300 of ~53,600 files, the largest bucket by a wide margin. Real scope, not yet designed — likely needs the same closure/env-struct machinery arrow functions already have, plus deciding how a named function expression's own name binds inside its own body (real JS lets it recurse by that name without the name leaking to the enclosing scope). |
| Calling the result of an expression directly (`f()()`, `(a ? f : g)()`) | `emitCall`'s callee dispatch only recognizes a bare identifier or a member expression — any other callee shape (another call's result, a parenthesized/conditional expression) is a clean "only simple function calls are supported" rejection. Second-largest bucket in the same conformance run, ~2,600 files. Confirmed independently (not just via the corpus) while validating TDD-00057's own nested-function work this session — a `make()()` pattern hit the identical rejection with zero test262 involvement. |
...
| `class` expressions (`const C = class { ... }`, optionally anonymous) | Only `class` *declarations* are supported — a class used in expression position is a clean parse-time rejection. ~1,400 files in the same conformance run. |
...
*Last updated: 2026-08-11 — Full repro-verification audit of every ✅ row on every page (prompted by finding optional parameters marked done while completely non-functional): found 33 further false/overclaiming ✅ claims, added honest caveats to all of them, and introduced a second **Strict Coverage** metric (zero known caveats/bugs, independently repro-verified) alongside the existing Coverage number everywhere in `docs/status/` — see [ADR-00166](../adr/ADR-00166.md).*
