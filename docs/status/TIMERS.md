...
|---|---|---|---|
| `setTimeout(fn, ms)` / `clearTimeout(id)` | ✅ | • Callback is restricted to a zero-argument, `void`-returning function — an arrow/function-expression closure, or a bare reference to a top-level named function ([ADR-00200](../adr/ADR-00200.md)) | • Bare global functions, matching real JS (not a namespace)<br>• The delay is clamped to `[1, 2^31-1]` ms as Node does — an omitted, `0`, negative or oversized delay is 1 ms, so a chain of zero-delay timers advances one millisecond per link (`setInterval` too; `setImmediate` is not clamped) ([ADR-01071](../adr/ADR-01071.md))<br>• See [ADR-00031](../adr/ADR-00031.md) |
