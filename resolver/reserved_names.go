...
// TDD-00049's own split). `-compat=js` opts back into real JS/
...
var reservedTier1 = map[string]bool{
	...
	"structuredClone": true, "Symbol": true, "BigInt": true,
}
...
// scope) to leave these reserved under both -compat modes rather than
...
// rejected only when allowGlobalShadowing is false (`-compat=strict`, the
...
func checkReservedBinding(name string, line, col int, allowGlobalShadowing bool) error {
	if reservedTier2[name] {
		return fmt.Errorf("%d:%d: '%s' is a reserved built-in name (constructed via `new %s(...)`) — this can never be shadowed, even under -compat=js, since the parser resolves `new %s(...)` to the built-in before any scope information exists",
			line, col, name, name, name)
	}
	if reservedTier1[name] && !allowGlobalShadowing {
		return fmt.Errorf("%d:%d: '%s' is a reserved built-in name — pass -compat=js to allow shadowing it (matches real JS/browser behavior)",
			line, col, name)
	}
	return nil
}
