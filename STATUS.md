...
| Unannotated function/arrow function **parameters** default to `i64` regardless of the actual argument type at a call site, with no coercion or check | `function log(msg) { console.log(msg) }; log("hello")` compiles clean but prints a garbage number instead of `hello` — the string pointer passed in gets bitwise-reinterpreted as an `i64` inside the callee, since the parameter's declared LLVM type doesn't match what's actually passed. Confirmed pre-existing (reproduces identically on the compiler from before `docs/adr/ADR-00041.md`'s return-type work) and confirmed fixed by adding an explicit `msg: string` annotation. Worse than every other entry in this table: silent data corruption, not a clean rejection or a crash. Deliberately **not fixed** as part of `ADR-00041.md` — it's a separate, sizable problem (parameter types, not return types) needing its own design decision (infer from call-site arguments? require annotations for non-numeric parameters and reject cleanly otherwise? something else?), not something to fold into an unrelated fix. |
...
| Fix | Effort | Notes |
|---|---|---|
| Unannotated function/arrow function **parameters** default to `i64` with no coercion or check against the actual call-site argument type | High | Silent data corruption (not a clean rejection), found while verifying `docs/adr/ADR-00041.md`. Needs its own design decision — infer from call-site usage, require annotations for non-numeric parameters, or something else — not a quick follow-on to the return-type work |
