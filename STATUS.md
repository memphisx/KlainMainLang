...
| Fix | Effort | Notes |
|---|---|---|
| `JSON.stringify` serializes a `Date` field as a raw ms number, not an ISO string | Low | Same switch, needs an `IsDate` case calling the existing `toISOString` formatter — same root-cause class as the (now-fixed, `docs/adr/ADR-00035.md`) float-truncation bug |
