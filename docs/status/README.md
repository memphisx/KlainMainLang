...
| Type system features | 16/23, ~70% | [TYPE-SYSTEM.md](TYPE-SYSTEM.md) | Generics V2 (opt-in type erasure) not started — see [TDD-00010](../tdd/TDD-00010.md); no intersection/tuple/mapped types |
...
*Last updated: 2026-08-07 (`TextEncoder`/`TextDecoder` and `structuredClone(obj)` shipped, closing out the Roadmap's entire former "Low effort" backlog tier — [ADR-00112](../adr/ADR-00112.md), [ADR-00113](../adr/ADR-00113.md); non-UTF-8 `TextDecoder` support scoped as a new staged, low-priority TDD — [TDD-00034](../tdd/TDD-00034.md); RegExp support — the last major "everyday JS" gap — scoped as a new staged TDD, PCRE2-backed — [TDD-00035](../tdd/TDD-00035.md), consolidating the former separate `String.prototype.matchAll()`/`RegExp` "not implemented" rows into one; RegExp Stage 0 (construction, literal syntax, field reads), Stage 1 (`.test()`), Stage 2 (`.exec()`, plus 3 general truthiness/null-comparison bug fixes it forced — bare ptr truthiness, array-vs-null comparison, string content-based truthiness), Stage 3 (`str.match()`/`str.matchAll()`, also correcting a TDD design mistake about global `.match()`'s zero-match return value), Stage 4 (`str.replace()`/`str.replaceAll()`, incl. `$1`-`$9`/`$&`/`$$` backreferences and an exactly-once-per-match callback form, narrowing the TDD's suggested callback shape to a fixed `(match, offset, string)`), Stage 5 (`str.split()`/`str.search()`, on a newly-needed stateless match primitive since real JS's own algorithms don't use `lastIndex` here), and Stage 6 (`--static` linking verified on bare Linux and in a real Docker `scratch` container, `docker/Dockerfile.regexp-test`) all shipped the same day, closing out RegExp support entirely — [ADR-00114](../adr/ADR-00114.md), [ADR-00115](../adr/ADR-00115.md), [ADR-00116](../adr/ADR-00116.md), [ADR-00117](../adr/ADR-00117.md), [ADR-00118](../adr/ADR-00118.md), [ADR-00119](../adr/ADR-00119.md), [ADR-00120](../adr/ADR-00120.md), new [REGEXP.md](REGEXP.md) status page). Previous update, 2026-08-06: nested-array storage, `.flat()`/`.flatMap()`, binary-safe `http.listen()` bodies, the `i64` MIN `/ -1` UB fix/`%=`, destructured function parameters, and getters/setters on classes shipped — TDD-00029/ADR-00105/ADR-00107, TDD-00026/ADR-00106, ADR-00108, ADR-00109, TDD-00030/ADR-00110; also fixed a stale [Design Documents](#design-documents-tdds) index — TDD-00026/00027/00029/00030 were missing/out of date — and added three new UI-direction TDDs: [TDD-00031](../tdd/TDD-00031.md) (terminal UI primitives, scoped and ready), [TDD-00032](../tdd/TDD-00032.md)/[TDD-00033](../tdd/TDD-00033.md) (GUI bindings / direct hardware access, bootstrapping placeholders only); `performance.mark`/`.measure` shipped as a same-day quick win — [ADR-00111](../adr/ADR-00111.md).*
