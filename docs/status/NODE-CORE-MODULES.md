...
**Strict Coverage**: 1/11, ~9% — a row only counts here if it was independently repro-verified with zero known caveats or bugs, of any severity. See the 2026-08-11 audit ([ADR-00166](../adr/ADR-00166.md)) that produced this number; no false ✅ claims found on this page — `assert` was already honestly caveated (no `.deepEqual`, `.throws` type-blind) before the audit.

**Caveats**:

- `net`/`dgram`/`tls`/`dns` (raw sockets) are not implemented — a prerequisite for a `WebSocket` built without going through `http.listen`'s existing accept loop (see [NETWORKING.md](NETWORKING.md)).
- `util` is the remaining CLI-relevant gap here (promisifying callback APIs, `.inspect`-style generic value formatting).
- `vm` has an opt-in embedded-engine path scoped in [TDD-00046](../tdd/TDD-00046.md) (low priority, not started); `cluster`/`http2`/`zlib` are listed for completeness, none close to being picked up.
