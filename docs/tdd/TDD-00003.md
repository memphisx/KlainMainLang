...
- **Status**: Superseded
- **Notes**: Superseded by [TDD-00230](TDD-00230.md) phase 4: fetch stays on libcurl, driven by libuv through its socket/timer callbacks; a future HTTP backend swap is a different `@lower` archive, not a helper process.
- **Date**: 2026-07-12

## Context

Came out of [ADR-00033](../adr/ADR-00033.md)'s `--static` + `fetch` investigation: getting a statically-linked `fetch`-using binary working on Alpine/musl needed curl's entire transitive dependency chain listed explicitly, a `gcc`-not-`clang` final link step to work around an LTO-format-incompatibility in two of Alpine's static archives, and a distro-specific CA-certificate-path fix — none of which is portable to other distros with any confidence. Worth asking directly: was `libcurl` ([ADR-00021](../adr/ADR-00021.md)) the right foundation for `fetch` at all, given this compiler's *own* toolchain is Go, and Go's `net/http`+`crypto/tls` stack is memory-safe and statically-linked (zero external C dependencies) by default with `CGO_ENABLED=0` — precisely the property this project fought hard to get out of `libcurl` in [ADR-00033](../adr/ADR-00033.md).
...
- **The core idea**: build a small, separate Go binary (`net/http` + `crypto/tls`, compiled with `CGO_ENABLED=0`) as part of building the KlainMainLang compiler itself, embed its bytes into the compiler via `//go:embed`, and — for a program that calls `fetch` — have the compiled native binary `fork`+`exec` that helper at runtime instead of calling into `libcurl` directly. This is *not* new ground: `execFileSync` ([ADR-00025](../adr/ADR-00025.md)) already proves the exact fork/exec/pipe mechanics needed (spawn a child process, capture its stdout, wait for it), just spawning a purpose-built helper instead of an arbitrary user-named one.
...
|---|---|---|
| Fork/exec/pipe process-spawning pattern | ✅ already proven | `execFileSync` ([ADR-00025](../adr/ADR-00025.md)) is a direct, verified precedent for exactly the mechanics needed |
