...
**270 / 289 features, ~93% coverage.**

**Strict Coverage** counts a feature only when its row's Caveats cell is empty (zero known caveats/bugs of any severity — the 2026-08-11 audit basis, [ADR-00166](../adr/ADR-00166.md)), over the same denominator as Coverage ([ADR-00205](../adr/ADR-00205.md)). Both figures now derive directly from each page's table per the [Status page format](#status-page-format), so a multi-category page (`LANGUAGE-CONSTRUCTS.md`, `TYPE-SYSTEM.md`) reports a real per-category split rather than one shared page-level number, and this index's figures match the page's.

| Category | Coverage | Strict | Page | Caveats |
|---|---|---|---|---|
| Control flow statements | 12/12, 100% | 10/12, ~83% | [LANGUAGE-CONSTRUCTS.md#control-flow-statements](LANGUAGE-CONSTRUCTS.md#control-flow-statements) | • A `for` loop's non-declaration init clause (`for (i = 0, j = 10; ...)`) is a clean rejection (the update clause does take comma-separated expressions — [ADR-00156](../adr/ADR-00156.md))<br>• No ASI restriction on a line break before a postfix `++`/`--`<br>• `finally` does not run on an early `return`/`break`/`continue` inside an `async`/generator body (it does everywhere else — [ADR-00191](../adr/ADR-00191.md)) |
| Operators | 22/22, 100% | 17/22, ~77% | [LANGUAGE-CONSTRUCTS.md#operators](LANGUAGE-CONSTRUCTS.md#operators) | • `**` uses exact i64 integer semantics — a negative exponent gives 0; use a float operand for fractional powers ([ADR-00187](../adr/ADR-00187.md))<br>• `&&`/`\|\|` yield a bool by default; `-compat=js` makes them value-preserving for same-typed operands ([ADR-00186](../adr/ADR-00186.md)/[ADR-00220](../adr/ADR-00220.md)) |
| Variable declarations | 3/3, 100% | 0/3, 0% | [LANGUAGE-CONSTRUCTS.md#variable-declarations](LANGUAGE-CONSTRUCTS.md#variable-declarations) | Definite-assignment is enforced for a typed `var`/`let` read before assignment — including `if`/`else`, `do/while`, and `switch`-with-`default` control flow — but the analysis is sound-not-complete: a binding assigned only inside a maybe-skipped `for`/`while` body, or in a `try` that may throw first, still escapes (reading its deterministic zero/`undefined` default — [ADR-00215](../adr/ADR-00215.md) — not uninitialized memory); correlated conditions (`if (c) {x=1} if (!c) {x=2}`) are conservatively *rejected*, matching TypeScript. See [TDD-00071](../tdd/TDD-00071.md)/[ADR-00210](../adr/ADR-00210.md)–[ADR-00215](../adr/ADR-00215.md) |
| Functions & closures | 10/10, 100% | 1/10, ~10% | [LANGUAGE-CONSTRUCTS.md#functions--closures](LANGUAGE-CONSTRUCTS.md#functions--closures) | • Nested function declarations are V1-scoped (no closure capture of the enclosing body's locals, one block deeper than the enclosing body is unsupported) — see [TDD-00057](../tdd/TDD-00057.md)<br>• Tagged template literals have no `.raw` property on the `strings` array (real `String.raw` stays separately out of scope, see [ADR-00028](../adr/ADR-00028.md)) — see [TDD-00059](../tdd/TDD-00059.md)/[ADR-00152](../adr/ADR-00152.md)<br>• A top-level function can't reference a sibling top-level `let`/`var`/`const` at all; a default parameter can't reference an earlier parameter; a rest-only arrow parameter list fails to parse — all found by the 2026-08-11 audit, see [ADR-00166](../adr/ADR-00166.md)<br>• Function expressions (anonymous and named) are supported, including self-referential/recursive named forms ([ADR-00178](../adr/ADR-00178.md)); the one residual is a named function expression whose name shadows a top-level function of the same name — a clean compile error (rename one) — see [TDD-00060](../tdd/TDD-00060.md)/[ADR-00168](../adr/ADR-00168.md)/[ADR-00178](../adr/ADR-00178.md)<br>• Generator functions are V1-scoped to a top-level declaration only, with no `yield*`/generator methods/async generators yet — see [TDD-00061](../tdd/TDD-00061.md)/[ADR-00173](../adr/ADR-00173.md) |
| Type primitives | 12/12, 100% | 7/12, ~58% | [TYPE-SYSTEM.md#type-primitives](TYPE-SYSTEM.md#type-primitives) | `bigint` now shipped ([TDD-00074](../tdd/TDD-00074.md)/[ADR-00216](../adr/ADR-00216.md)), selectable `-bigint=libtommath\|gmp`; residual V1 caveats — no `.toLocaleString`/`asIntN`/`BigInt64Array` (bigint↔float comparison is now a `-compat` design choice, [TDD-00075](../tdd/TDD-00075.md), not a caveat) |
| Async / Promise | 3/6, 50% | 0/6, 0% | [LANGUAGE-CONSTRUCTS.md#async--promise](LANGUAGE-CONSTRUCTS.md#async--promise) | • Only `await fetch(...)` is genuinely non-blocking; every other `Promise<T>` is a resolved-slot read<br>• No `Promise.any`, async generators, or `for await...of` |
| String methods | 27/29, ~93% | 19/29, ~66% | [STRING-METHODS.md](STRING-METHODS.md) | • No `.normalize()`<br>• `.charCodeAt()`/`.codePointAt()` read garbage memory out-of-range instead of returning `NaN` — found by the 2026-08-11 audit, see [ADR-00166](../adr/ADR-00166.md) |
...
| Array methods | 35/35, 100% | 22/35, ~63% | [ARRAY-METHODS.md](ARRAY-METHODS.md) | • `.flat(depth?)`/`.flatMap(fn)`: `depth` must be a compile-time constant (or `Infinity`), since this compiler's array element types are static ([TDD-00029](../tdd/TDD-00029.md)/[ADR-00107](../adr/ADR-00107.md))<br>• `Array.from` supports the array-like overload only<br>• `.pop()`/`.shift()` corrupt state on an empty array; `.push`/`.pop`/`.shift`/`.unshift`/`.splice` all fail on `this.field.push(x)` — found by the 2026-08-11 audit, see [ADR-00166](../adr/ADR-00166.md) |
| Number / Math | 35/35, 100% | 23/35, ~66% | [NUMBER-MATH.md](NUMBER-MATH.md) | • `Math.floor/ceil/round/trunc/min/max/sign` produce undefined-behavior garbage on `NaN`/`±Infinity` input<br>• `parseInt`/`parseFloat` never return `NaN` — found by the 2026-08-11 audit, see [ADR-00166](../adr/ADR-00166.md) |
...
| JSON | 13/15, ~87% | 10/15, ~67% | [JSON.md](JSON.md) | • `JSON.stringify` now does `space` pretty-printing (`JSON.stringify(x, null, 2)`) and honors a class `toJSON()` ([ADR-00222](../adr/ADR-00222.md)); a runtime `space` and a non-null `replacer` are clean rejections<br>• `JSON.parse` now validates input and throws a catchable `SyntaxError` on malformed JSON ([ADR-00223](../adr/ADR-00223.md)), and projects nested objects, array & object-array fields, top-level `T[]`, and float variables off a parse tree ([ADR-00224](../adr/ADR-00224.md)) — retiring the old flat-only extractor, the array-field rejection, and the float-variable compile bug<br>• `JSON.parse` → `any`/`unknown` (dynamic shape) is the remaining gap ([TDD-00077](../tdd/TDD-00077.md) Track P P4); a bare reassignment `xs = JSON.parse(...)` lacks target-type context |
| console | 11/12, ~92% | 7/12, ~58% | [CONSOLE.md](CONSOLE.md) | • No `console.table()`<br>• `console.warn` has an undocumented prefix; `console.trace` prints no stack — found by the 2026-08-11 audit |
| Global functions & constants | 14/17, ~82% | 7/17, ~41% | [GLOBAL-FUNCTIONS.md](GLOBAL-FUNCTIONS.md) | • No `queueMicrotask`; `eval` has an opt-in embedded-engine path scoped in [TDD-00046](../tdd/TDD-00046.md), not started<br>• `parseInt`/`parseFloat` never return `NaN`<br>• `NaN`/`Infinity` shadowing needs `-compat=js`, not unconditional as previously documented — found by the 2026-08-11 audit |
| Type system features | 10/12, ~83% | 6/12, 50% | [TYPE-SYSTEM.md#type-system-features](TYPE-SYSTEM.md#type-system-features) | • Generics are unconstrained type parameters only, with no explicit call-site type arguments ([TDD-00010](../tdd/TDD-00010.md), [TDD-00037](../tdd/TDD-00037.md))<br>• Union types (beyond `T \| null`) are scalar-members-only — not nested inside a container, no flow-based narrowing ([TDD-00043](../tdd/TDD-00043.md))<br>• Tuple types are fixed-shape structs ([TDD-00066](../tdd/TDD-00066.md)/[ADR-00201](../adr/ADR-00201.md))<br>• No intersection/mapped types |
| Classes / OOP | 15/16, ~94% | 9/16, ~56% | [LANGUAGE-CONSTRUCTS.md#classes--oop](LANGUAGE-CONSTRUCTS.md#classes--oop) | • No user-definable `class X extends Error` (built-in types aren't valid `extends` targets, by design)<br>• Clean rejections: static-field initializers, static/async generator methods, a dynamic/`Symbol.*` computed member key, and a class expression used as a runtime value<br>• A field initializer can currently see constructor params (real JS's separate initializer scope forbids this — [ADR-00180](../adr/ADR-00180.md))<br>• `#x` private names have no early-error check for `#m`/`static #m` name collision or the `#constructor`-is-banned rule ([ADR-00155](../adr/ADR-00155.md))<br>• A string enum assigned to a typed variable fails to compile — 2026-08-11 audit, [ADR-00166](../adr/ADR-00166.md) |
...
| Binary data & Typed Arrays | 2/9, ~22% | 1/9, ~11% | [BINARY-DATA-TYPED-ARRAYS.md](BINARY-DATA-TYPED-ARRAYS.md) | • No `DataView`/`Blob`/`SharedArrayBuffer`/`Atomics`<br>• No `BigInt64Array`/`BigUint64Array` (`bigint` now exists — [TDD-00074](../tdd/TDD-00074.md) — but the typed arrays aren't built on it yet)<br>• No Node `Buffer` |
...
| Networking (fetch, WebSocket, SSE) | 6/6, 100% | 1/6, ~17% | [NETWORKING.md](NETWORKING.md) | • `WebSocket` (both server and client, [TDD-00039](../tdd/TDD-00039.md)) has no binary send and no `wss://`/TLS<br>• `XMLHttpRequest` ([TDD-00040](../tdd/TDD-00040.md)) is legacy-synchronous-mode only<br>• `.text()`/`.json()` still truncate at an embedded null byte by design — use `.arrayBuffer()` for binary bodies ([ADR-00094](../adr/ADR-00094.md))<br>• `await`-ing `Response.text()`/`.json()`/`.arrayBuffer()` (they're actually synchronous) hard compile-crashes and frees a live buffer — found by the 2026-08-11 audit, see [ADR-00166](../adr/ADR-00166.md) |
...
| Events & Cancellation | 0/3, 0% | 0/3, 0% | [EVENTS-CANCELLATION.md](EVENTS-CANCELLATION.md) | • Not started; blocks a general `AbortController`<br>• Distinct from Node's `EventEmitter`, see [EVENT-EMITTER.md](EVENT-EMITTER.md) below |
...
| Dynamic `import()` — real lazy / runtime-conditional loading | The eager literal-specifier form and `import.meta.url` already shipped ([ADR-00148](../adr/ADR-00148.md)); only genuine laziness remains — see [MODULES.md](MODULES.md)/[TDD-00055](../tdd/TDD-00055.md)/[TDD-00056](../tdd/TDD-00056.md) |
...
| [00008](../tdd/TDD-00008.md) External Conformance Suites | Partially Implemented, ongoing | V1's hand-curated notes: [`docs/testing/CONFORMANCE-COVERAGE.md`](../testing/CONFORMANCE-COVERAGE.md). V2's real, generated, full-corpus numbers: [`docs/testing/CONFORMANCE-RESULTS.md`](../testing/CONFORMANCE-RESULTS.md) — 5,225/53,578 (9.8%) on the Linux baseline. A gap it surfaced that is still open: spread in call arguments, tracked under [What Is NOT Implemented](#what-is-not-implemented). (Generic object stringification — `` `${obj}` `` for a plain class instance — is now done via the object inspector, [ADR-00218](../adr/ADR-00218.md).) |
...
- **This index links to the anchor** — each category row's Page cell points at that category's section (`PAGE.md#category`) and keeps its own one-line Coverage/Strict figure matching the page; the page is the source of truth if the two ever drift.

Every detail page linked from this index follows this format.

*Last updated: 2026-08-18 — added the shared [Status page format](#status-page-format) convention (binary Status, split Caveats/Notes columns, per-category tables with index anchors); TYPE-SYSTEM.md is the reference implementation. Test262 corpus at 5225/53578 (9.8%) on the Linux baseline.*
