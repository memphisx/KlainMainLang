...
| Operators | 42/42, 100% | 34/65, ~52% (page) | [LANGUAGE-CONSTRUCTS.md](LANGUAGE-CONSTRUCTS.md) | `**` doesn't exist at the lexer level; `&&`/`\|\|` don't actually short-circuit (both operands always evaluate); `NaN`'s `!=`/`!==` comparisons are wrong — all found by the 2026-08-11 audit, see [ADR-00166](../adr/ADR-00166.md) |
...
| `class` expressions (`const C = class { ... }`, optionally anonymous) | Only `class` *declarations* are supported — a class used in expression position is a clean parse-time rejection. ~1,400 files in the same conformance run. Now scoped, alongside the rest of the class-body member surface (field initializers, async/generator/computed methods), in [TDD-00063](../tdd/TDD-00063.md) — the single largest in-scope conformance gap (7,166 failing class files, 37% of all `language/` failures). |
...
*Last updated: 2026-08-13 — computed class member names with a constant string/number key (`['foo']() {}`) now work, desugared to the equivalent named member ([TDD-00063](../tdd/TDD-00063.md) Stage 3 / [ADR-00184](../adr/ADR-00184.md)).*
