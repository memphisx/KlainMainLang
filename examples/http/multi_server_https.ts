// A plain HTTP primary server plus an HTTPS server on a second port
// (TDD-00191 Stage 3 / ADR-00806). Each additional listener carries its own TLS
// context in the extra-listener table, so its accepts do their own SSL
// handshake while the primary stays plain — an app that serves plain HTTP on one
// port and HTTPS on another, in one process. Try the HTTPS port with:
//   curl -k https://127.0.0.1:<port>/hello
//
// { cert, key } are PEM strings (a throwaway self-signed pair, inline so the
// example is self-contained). A self-closing sequence keeps it runnable to
// completion.
import http from 'http';
import type { AddressInfo } from 'net';
import https from 'https';

const cert = "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n";
const key = "-----BEGIN PRIVATE KEY-----\\n4YE1nJLFlTbbvxexv4mGXDIT\n-----END PRIVATE KEY-----\n";

const plain = http.createServer((req: http.IncomingMessage, res: http.ServerResponse) => {
  res.writeHead(200, { "Content-Type": "text/plain" });
  res.end("plain http: " + req.url);
});

const secure = https.createServer({ cert: cert, key: key }, (req, res) => {
  res.writeHead(200, { "Content-Type": "text/plain" });
  res.end("https/1.1: " + req.method + " " + req.url);
});

plain.listen(0, () => {
  secure.listen(0, () => {
    console.log("plain http on port", (plain.address() as AddressInfo).port > 0);
    console.log("https on port", (secure.address() as AddressInfo).port > 0);
    plain.close();
    secure.close();
  });
});

console.log("plain http + https server on a second port");
