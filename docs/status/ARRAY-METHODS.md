...
| `.find(fn)` | ✅ | | • Absence result is a real `T | undefined` (TDD-00187 Stage 1, [ADR-00778](../adr/ADR-00778.md)): strict mode requires narrowing/`??`/`!` at bare-`T` boundaries, `-compat=js` auto-widens |
| `.findIndex(fn)` | ✅ | | |
| `.some(fn)` | ✅ | | |
| `.every(fn)` | ✅ | | |
| `.map(fn)` | ✅ | | |
