...
- **Status**: Implemented (see [ADR-00031](../adr/ADR-00031.md))
- **Date**: 2026-07-12

## Context

`setTimeout`/`clearTimeout`/`setInterval`/`clearInterval` were tracked in `STATUS.md` as needing "an event loop" — but that overstated the prerequisite. Timers only need a much smaller piece of infrastructure than the general-purpose, I/O-multiplexing event loop [TDD-00006](TDD-00006.md) scopes out for the HTTP server: a sorted-by-fire-time queue and a loop that sleeps until the next one is due, with no socket/file-descriptor readiness polling involved at all. Written up before any code was implemented, specifically to confirm that smaller-than-assumed scope before starting.
...
| General-purpose (I/O-multiplexing) event loop ([TDD-00006](TDD-00006.md)) | **not needed for this feature at all** | Timers only ever need a sleep-until-next-due loop, a meaningfully smaller problem than what [TDD-00006](TDD-00006.md) scopes |
| Garbage collection / memory management ([TDD-00001](TDD-00001.md)) | ⚠️ not a blocker to *start* | A `setInterval` that runs for a program's entire (now possibly long) lifetime allocates a fresh closure environment on every fire if the callback itself allocates — for a short-lived demo this is a non-issue; for a long-running interval-heavy program it's the same "never free anything" concern [TDD-00001](TDD-00001.md) tracks |

## Open questions

None outstanding at the time this was written — the design above was implemented essentially as scoped; see [ADR-00031](../adr/ADR-00031.md) for what actually shipped (including the one detail that changed: the entry-layout's `intervalMs = -1` cancellation sentinel, decided during implementation rather than in this document) and the real bugs found and fixed along the way.
