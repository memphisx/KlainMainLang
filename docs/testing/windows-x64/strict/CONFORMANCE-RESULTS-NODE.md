...
3957 files total: **56 passed**, 2530 failed, 1371 skipped (out of scope).

Of the 2586 files that compiled far enough to run, **56 passed (2.2%)**.
...
| 23 | COMPILE_ERROR: '%s' is not an array |
...
| 12 | COMPILE_ERROR: an Http2Stream supports .respond(headers), .write(chunk), .end(chunk?), .on('%s'|'%s'… |
| 12 | COMPILE_ERROR: an http.Server supports .on('%s'|'%s'|'%s'|'%s'|'%s'|'%s'|'%s'|'%s', listener) (got '… |
...
|---|---|---|
| `es-module/test-esm-preserve-symlinks.js` | esm |  |
| `es-module/test-esm-symlink-main.js` | esm |  |
...
| `parallel/test-child-process-send-utf8.js` | child |  |
...
| `parallel/test-path-zero-length-strings.js` | path | −8 |
