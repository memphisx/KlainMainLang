...
|---|---|---|---|
| `console.log(...)` | ✅ | • A string with an embedded null byte truncates *on display* at the first `\0` (`printf` `"%s"`); the stored string is intact. No binary-safe print — use `process.stdout.write` for binary output | • Multiple arguments join with single spaces on one line, a no-arg call prints a bare newline, and `-0` displays as `-0` (Node's util.inspect display; `String(-0)` stays `"0"`) — [ADR-00285](../adr/ADR-00285.md). Binary-safe `fwrite` print deferred as display-only (see [TDD-00120](../tdd/TDD-00120.md), [ADR-00364](../adr/ADR-00364.md))<br>• On Windows, VT/ANSI output processing is enabled on the console at startup (as Node does), so an escape like `\x1b[31m` renders as colour rather than printing literally — no raw mode required ([ADR-00744](../adr/ADR-00744.md))<br>• On Windows, the console is set to UTF-8 at startup (restored at exit), so non-ASCII output (`café`, CJK, emoji) renders correctly instead of OEM-code-page mojibake; redirected/piped output is unaffected either way ([ADR-00747](../adr/ADR-00747.md)) |
...
| `console.group()` / `.groupEnd()` | ✅ | | • Indents every subsequent `console.*` line by two spaces per nesting level; an unbalanced extra `groupEnd()` floors at depth 0 rather than going negative. See [ADR-00029](../adr/ADR-00029.md).<br>• `console.group(...label)` takes variadic label args, formatted like `console.log`; `console.groupCollapsed` is the terminal-identical alias (a browser devtools console would render it collapsed) ([ADR-00792](../adr/ADR-00792.md)) |
