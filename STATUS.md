...
| `Date` | ✅ | `new Date()` / `new Date(ms)` / `new Date(isoString)` (the string form parses via the same logic as `Date.parse`, including its `-1`-on-unparseable sentinel — see `docs/adr/ADR-00038.md`); `getFullYear/Month/Date/Day/Hours/Minutes/Seconds/Milliseconds`, `getTime`/`valueOf`, `toISOString` — all UTC, not local time, for deterministic output regardless of the machine/CI timezone (a documented deviation from real JS's local-time default). See `docs/adr/ADR-00014.md`. |
...
| `new Date(year, month, day, ...)` (the multi-argument constructor) is entirely unimplemented | `new Date(2023, 10, 14)` fails with a parse error (`expected ), got ,`) — a clean failure, since `parseNewDateBody` only ever accepts zero or one argument. Real JS's multi-arg form (year, month, day, hours, minutes, seconds, ms, with month 0-indexed) isn't accepted in any form yet. |
