...
| Count | Reason |
|---|---|
| 220 | COMPILE_ERROR: a number has no method '%s' |
...
| 54 | COMPILE_ERROR: field access on non-object (no field '%s') |
...
| 37 | COMPILE_ERROR: capturing array variable '%s' in a closure is not yet supported |
| 36 | MODULE_NOT_IMPLEMENTED: domain |
| 25 | COMPILE_ERROR: process has no method '%s' |
| 23 | COMPILE_ERROR: an http2 '%s' listener must be (stream, headers[, flags]) => void |
| 23 | MODULE_NOT_IMPLEMENTED: perf_hooks |
| 23 | RUNTIME_NONZERO_EXIT:  |
