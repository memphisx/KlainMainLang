...
| Binary data & Typed Arrays | 9/9, 100% | 1/9, ~11% | [BINARY-DATA-TYPED-ARRAYS.md](BINARY-DATA-TYPED-ARRAYS.md) | • TypedArray construction is restricted to a variable declaration's initializer (Node `Buffer`'s call-syntax statics are exempt); no `.buffer`<br>• BigInt64Array/BigUint64Array support an explicit method allow-list — everything else is a compile-time rejection<br>• No `Blob.stream()`; Buffer has no `utf16le`, and encodings must be string literals<br>• No `Atomics.waitAsync`, no `getFloat16` |
