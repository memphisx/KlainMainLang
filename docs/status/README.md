...
| Variable declarations | 4/4, 100% | 35/65, ~54% (page) | [LANGUAGE-CONSTRUCTS.md](LANGUAGE-CONSTRUCTS.md) | No `let`/`const`-vs-`var` scoping distinction — no TDZ/hoisting or redeclaration checks (all three are mutable allocas) |
...
*Last updated: 2026-08-17 — cleared the two now-fixed Variable-declarations caveats (a missing `const` initializer and `eval`/`arguments` as a strict-mode binding name are both enforced now — [ADR-00209](../adr/ADR-00209.md)). Test262 corpus at 5225/53578 (9.8%) on the Linux baseline.*
