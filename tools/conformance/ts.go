...
func runTSSuite(workDir string, timeout time.Duration, compat string) {
	...
	// (docs/testing/<lane>/CONFORMANCE-RESULTS-TS.md), so a per-flag regression
	// is a clean per-file diff (TDD-00022). The js lane accepts more untyped
	// patterns, so it typically trades false-rejects for false-accepts vs TS.
	var lanes []string
	...
	var primary []tsResult
	for _, lane := range lanes {
		laneCompat = lane
		results := runTSLane(casesRoot, hasErrorsBaseline, workDir)
		primary = results
		out := reportPath("CONFORMANCE-RESULTS-TS.md")
		if err := writeTSReport(out, results); err != nil {
			fatal("writing TS report: %v", err)
		}
		fmt.Fprintf(os.Stderr, "ts suite (lane=%s): %d cases, report written to %s\n", laneLabel(), len(results), out)
	}
...
func writeTSReport(path string, all []tsResult) error {
	...
	if err := updateConformanceSummary("ts", laneLabel(), tsSummaryLane{
		Agree:       agree,
