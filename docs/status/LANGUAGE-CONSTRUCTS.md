...
| Destructured function parameters (`function f({ x, y }: T) {}` / `function f([a, b]: T[]) {}`) | ✅ | V1 scope: a required explicit type annotation on the pattern (no sensible unannotated default the way a scalar param has), no nesting, no combination with `...`/a whole-parameter default value. Works on named function declarations, class methods/constructors, and arrow functions — array-destructured params are the one combination not supported on an arrow function/closure specifically, since array-typed closure parameters aren't supported at all yet (a separate, pre-existing gap, not new here). See [ADR-00109](../adr/ADR-00109.md). |
