...
| Typed calls + marshalling (all scalar types, `string`, `pointer`, `buffer`, `arraybuffer`, 64-bit ↔ `bigint`) | ✅ | • `int64`/`uint64` parameters also accept a plain `number` (Node requires `bigint`); `bool` marshals as 0/1 numbers, matching Node<br>• A `function`-typed value can be passed through as a raw `bigint` address; to hand C a callable made from a closure, use `library.registerCallback([sig,] cb)` (its own row below), which emits a real C-ABI function pointer | |
