...
| Operators | 22/22, 100% | 17/22, ~77% | [LANGUAGE-CONSTRUCTS.md](LANGUAGE-CONSTRUCTS.md) | • `**` uses exact i64 integer semantics — a negative exponent gives 0; use a float operand for fractional powers ([ADR-00187](../adr/ADR-00187.md))<br>• `&&`/`\|\|` yield a bool by default; `-compat=js` makes them value-preserving for same-typed operands ([ADR-00186](../adr/ADR-00186.md)/[ADR-00220](../adr/ADR-00220.md)) |
...
| Type primitives | 12/12, 100% | 8/12, ~67% | [TYPE-SYSTEM.md](TYPE-SYSTEM.md) | `bigint` now shipped ([TDD-00074](../tdd/TDD-00074.md)/[ADR-00216](../adr/ADR-00216.md)), selectable `-bigint=libtommath\|gmp`; residual V1 caveats — no `.toLocaleString`/`asIntN`/`BigInt64Array` (bigint↔float comparison is now a `-compat` design choice, [TDD-00075](../tdd/TDD-00075.md), not a caveat) |
| Async / Promise | 3/6, 50% | 0/6, 0% | [LANGUAGE-CONSTRUCTS.md](LANGUAGE-CONSTRUCTS.md) | • Only `await fetch(...)` is genuinely non-blocking; every other `Promise<T>` is a resolved-slot read<br>• No `Promise.any`, async generators, or `for await...of` |
