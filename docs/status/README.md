...
| Binary data & Typed Arrays | 5/9, ~56% | 1/9, ~11% | [BINARY-DATA-TYPED-ARRAYS.md](BINARY-DATA-TYPED-ARRAYS.md) | • TypedArray construction is restricted to a variable declaration's initializer; no `.buffer`, no sub-range (3-argument) views<br>• No `Blob`<br>• No `BigInt64Array`/`BigUint64Array`<br>• No Node `Buffer` |
...
- **Linux `-mm=gc` + `Worker` segfault** — the GC-mode worker round trip (`TestE2EWorkerGCModeRoundTrip`) segfaults on Linux glibc/arm64 (Docker), while every other worker/shared-memory test passes there. The per-thread Boehm registration + fiber stack-bottom path ([ADR-00306](../adr/ADR-00306.md)) was only ever verified on the Mac. See [CONCURRENCY-WORKERS.md](CONCURRENCY-WORKERS.md).
...
- Node `Buffer` — a `Uint8Array` subclass with Node's extra methods; rides the existing `ArrayBuffer`/TypedArray machinery. See [BINARY-DATA-TYPED-ARRAYS.md](BINARY-DATA-TYPED-ARRAYS.md).
