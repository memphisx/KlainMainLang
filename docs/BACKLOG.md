...
**Total: 652 caveats · overall Strict Coverage 311/612 (~51%).** Areas below are ordered worst Strict Coverage first.
...
### Other Node.js Core Modules — Strict 1/17 (~6%) · 69 caveats — [Other Node.js Core Modules](status/NODE-CORE-MODULES.md)
...
- `cluster` — `setupPrimary`'s `exec`/`execArgv` are a compile-time rejection — workers re-exec this compiled program itself, so there is no separate script file for a worker to run (a genuine impossibility of native compilation, [ADR-01015](adr/ADR-01015.md)); `silent` is POSIX-only until the Windows spawn lane lands ([TDD-00177](tdd/TDD-00177.md))
...
### Type System — Strict 12/31 (~39%) · 44 caveats — [Type System](status/TYPE-SYSTEM.md)
...
### Language Constructs — Strict 48/78 (~62%) · 64 caveats — [Language Constructs](status/LANGUAGE-CONSTRUCTS.md)
