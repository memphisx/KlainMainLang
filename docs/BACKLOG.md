...
- **IOCP reactor remaining stages** (TDD-00183, Stages 1–2 done — sockets and
  pipes are on the port, no poll slice remains): Stage 3 refcounted
  descriptions (`dup2` aliasing, the 384-socket cap); Stage 4 hi-res timers; fs
  thread pool onto the port. Plus `cluster` round-robin (TDD-00177/00105, 3
  skipped tests).
...
**Total: 650 caveats · overall Strict Coverage 311/611 (~51%).** Areas below are ordered worst Strict Coverage first.
