...
**Caveats**: The shipped event loop ([TDD-00006](../tdd/TDD-00006.md)) is cooperative, one-fiber-at-a-time concurrency, not preemptive multi-threading — `Worker` needs a genuinely separate mechanism (`pthreads`), plus `SharedArrayBuffer`/`Atomics` first.
