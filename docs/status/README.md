...
| Global functions & constants | 18/20, 90% | 8/20, 40% | [GLOBAL-FUNCTIONS.md](GLOBAL-FUNCTIONS.md) | • `eval` is an opt-in embedded-engine path ([TDD-00046](../tdd/TDD-00046.md)), not started<br>• `parseInt` lacks hex auto-detect with an omitted radix<br>• `NaN`/`Infinity` shadowing needs `-compat=js`, not unconditional |
...
| Binary data & Typed Arrays | 3/9, ~33% | 1/9, ~11% | [BINARY-DATA-TYPED-ARRAYS.md](BINARY-DATA-TYPED-ARRAYS.md) | • No `Blob`/`SharedArrayBuffer`/`Atomics`<br>• No `BigInt64Array`/`BigUint64Array`<br>• No Node `Buffer` |
