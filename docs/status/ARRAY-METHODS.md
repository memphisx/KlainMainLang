...
**Caveats**: `.flat()`/`.flatMap()` are blocked on nested-array support — `number[][]`-style literals aren't reliably representable yet (`[[1,2],[3,4]]` fails to compile). `Array.from(iterable)` isn't implemented yet, though the iterable protocol it would drain (a class implementing `next(): T | null`) already exists.
...
| `Array.isArray(x)` | ✅ |
| `Array.from(iterable)` | ❌ (a user-extensible `for...of` iterable protocol now exists — a class implementing `next(): T \| null`, see [ADR-00063](../adr/ADR-00063.md) — but `Array.from` itself, which would drain such an iterable into a real array, isn't implemented yet) |
