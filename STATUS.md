...
- **[Event Loop](docs/tdd/TDD-00006.md)** — this project's single biggest structural gap. Part 1 (the `select()`-based wait loop) done (`ADR-00048`); Part 2's fiber-based scheduler is real and shipped for HTTP connection concurrency (`ADR-00049`), but `async`/`await`/`fetch()` themselves still run fully synchronously — real concurrent `await fetch(...)` needs libcurl's non-blocking multi-interface too, not yet started.
...
2. **Event loop — Part 1 and a first real Part 2 slice both done; `async`/`await`/`fetch()` itself still not** (see [`docs/tdd/TDD-00006.md`](docs/tdd/TDD-00006.md)). Part 1 (a `select()`-based wait loop merging with the existing timer queue) shipped alongside the HTTP server below — see `docs/adr/ADR-00048.md`. Part 2 (real suspension) was scoped around three candidate mechanisms; a direct prototyping spike ruled out LLVM coroutine intrinsics (confirmed incompatible with this compiler's `setjmp`/`longjmp` exception model — a `try`/`catch` spanning a suspend point segfaults) and confirmed hand-rolled fibers (`ucontext.h`, no custom assembly needed) work correctly instead — see `docs/adr/ADR-00049.md`, which used that confirmed mechanism to make `http.listen` handle connections concurrently (each connection is its own fiber, yielding on a would-block read instead of stalling every other connection). `async`/`await`/`fetch()` themselves are unaffected by this and still run fully synchronously — real concurrent `await fetch(...)` additionally needs libcurl's non-blocking multi-interface, a separate, not-yet-started piece of work.
