...
Windows 11 x86-64 is supported through the mingw-w64 UCRT toolchain from MSYS2 (the design and its caveats are in [docs/tdd/TDD-00177.md](docs/tdd/TDD-00177.md)). The compiler targets `x86_64-w64-mingw32` and links against the same Universal C Runtime every Windows 10+ install already has, so compiled programs need no extra runtime. Install, in this order:

1. Git for Windows (`winget install Git.Git`) — the Makefile and the test scripts assume its Git Bash.
