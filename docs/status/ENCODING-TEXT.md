...
**Coverage**: 2/2 (100%) · **Strict Coverage**: 0/2 (0%).
...
|---|---|---|---|
| `TextEncoder` | ✅ | • Only `.encode()` is implemented — `.encodeInto(src, dest)` is a compile error and there is no `.encoding` property (real `TextEncoder` exposes both). | • `new TextEncoder().encode(str): Uint8Array` — UTF-8 only, which matches the spec (`TextEncoder` is UTF-8-only by spec, not a narrowing)<br>• Strings are already raw UTF-8 byte sequences, so `encode`/`decode` are direct byte copies with no real transcoding<br>• See [ADR-00112](../adr/ADR-00112.md) |
