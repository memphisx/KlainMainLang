...
| Array reads/writes have no bounds check against the array's actual length | `const arr = [1, 2, 3]; arr[10] = 99;` leaves `arr.length` at 3 (no growth/resize), but a subsequent `arr[10]` correctly reads back `99` — confirmed, via write-then-read-back, that the write physically lands past the allocated 3-element buffer in heap memory. `emitIndexPtr` (`codegen/llvm/emit_exprs.go`) computes a raw `getelementptr` off the array's data pointer at the given index, for both the named-symbol and aggregate-value forms, with no comparison against the stored length anywhere in the read or write path. Real, unchecked out-of-bounds heap read/write — the most severe of the bugs found in this pass. |
| `const` reassignment is silently permitted | `const x = 5; x = 10; console.log(x)` compiles and prints `10`, instead of the compile-time error real TS/JS gives (`TypeError: Assignment to constant variable`). Root cause: `Symbol` (`codegen/llvm/emitter.go`) has no field recording a binding's declared kind at all — `decl.Kind` (`"let"`/`"const"`/`"var"`) is parsed and available but never propagated onto the `Symbol` created for it, so `emitAssign` has nothing to check a reassignment against. |
| Bitwise shift operators (`<<`, `>>`, `>>>`) use raw native 64-bit semantics, not JS's 32-bit ones | `1 << 63` prints `-9223372036854775808` here; real JS gives `-2147483648`, since the spec requires converting both operands to 32-bit integers (`ToInt32`/`ToUint32`) and masking the shift count to 0-31 before shifting. `emit_exprs.go`'s `<<`/`>>`/`>>>` cases coerce both operands straight to `i64` and emit plain `shl`/`ashr`/`lshr` with no truncation step, so results diverge from real JS for any shift count ≥ 32 or any operand outside the 32-bit range, and shift counts ≥ 64 are likely undefined behavior at the LLVM level (not yet confirmed either way). |
| `(FuncType)[]` — an array-of-function-type annotation — silently mis-parses instead of being rejected or supported | `let counters: (() => number)[] = [];` fails with the confusing, unrelated-sounding codegen error `can only assign to identifiers or array elements`, rather than a parse error or working correctly. Root cause is in the parser, not codegen: `parseTypeAnnotation`'s `LPAREN` branch (`parser/parser.go`) returns immediately once it finishes parsing the parenthesized function type, without checking for a trailing `[]`, unlike the sibling object-type-annotation branch (`LBRACE`) which explicitly loops to support `{...}[]`. The declaration silently splits into two bogus statements — a truncated `let counters: (() => number)` (annotation cut short, no initializer, no error) followed by a second "statement" starting at the leftover `[] = [];`, which parses as a grammatically valid assignment (empty-array-literal left-hand side) and only fails much later, at codegen, with no connection back to the real cause. |
...
- **[External Conformance Suites (TypeScript + Test262) as a Test-Coverage Benchmark](docs/tdd/TDD-00008.md)** — the TypeScript suite tests the type checker's output, not runtime behavior, so it can't be used directly; Test262 turned out to be execution-based and often directly portable instead, at least for spec-mandated value semantics this compiler intends to match. Tracked in [`docs/testing/CONFORMANCE-COVERAGE.md`](docs/testing/CONFORMANCE-COVERAGE.md). Scoped, not started.
...
| Fix | Effort | Notes |
|---|---|---|
| Array reads/writes have no bounds check | Small–medium | Highest-priority of the four below: real, unchecked heap corruption, not just wrong output. A minimal fix (compare index against the stored length, throw/abort on out-of-range) is self-contained to `emitIndexPtr`/`emitIndex`; deciding what happens on violation (hard abort vs. a catchable `Error`, matching this compiler's existing exception mechanism) is the only open design question |
| `const` reassignment is silently permitted | Small | Needs a `Symbol.IsConst` (or similar) field set from `decl.Kind` at declaration time, checked in `emitAssign` before allowing a plain `=` (not `+=` etc., which already require prior assignment); self-contained, no cross-cutting design questions |
| `(FuncType)[]` array-of-function-type annotations mis-parse instead of erroring or working | Small | Parser-only fix: teach `parseTypeAnnotation`'s `LPAREN` branch to check for a trailing `[]` the same way the `LBRACE` (object-type) branch already does, wrapping the parsed function type in an array `TypeAnnotation` |
| Bitwise shifts (`<<`, `>>`, `>>>`) use 64-bit semantics instead of JS's 32-bit ones | Medium | Needs both operands truncated to 32-bit (`ToInt32`/`ToUint32`) and the shift count masked to 0-31 before the actual `shl`/`ashr`/`lshr`, matching the JS spec; also needs deciding what the *result* type should be (JS shift results are always 32-bit, but this compiler's `number` is a 64-bit `i64`) |
